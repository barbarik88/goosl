...
const symbolPool = [
  { type: 'multiplier', label: 'x1', multiplier: 1, tier: 'low', weight: 14 },
  { type: 'multiplier', label: 'x2', multiplier: 2, tier: 'low', weight: 18 },
  { type: 'multiplier', label: 'x3', multiplier: 3, tier: 'low', weight: 16 },
  { type: 'multiplier', label: 'x5', multiplier: 5, tier: 'low', weight: 12 },
  { type: 'multiplier', label: 'x10', multiplier: 10, tier: 'medium', weight: 9 },
  { type: 'multiplier', label: 'x20', multiplier: 20, tier: 'medium', weight: 6 },
  { type: 'multiplier', label: 'x75', multiplier: 75, tier: 'high', weight: 3 },
  { type: 'multiplier', label: 'x150', multiplier: 150, tier: 'high', weight: 2 },
  { type: 'multiplier', label: 'x200', multiplier: 200, tier: 'high', weight: 1 },
  { type: 'win', label: 'WIN\n€20', amount: 20, weight: 4 },
  { type: 'win', label: 'WIN\n€50', amount: 50, weight: 2 },
  { type: 'bonus', label: 'BONUS\n+€50', amount: 50, weight: 4 },
  { type: 'bonus', label: 'BONUS\n+€100', amount: 100, weight: 2 },
  { type: 'free-spins', label: '+5 FS', amount: 5, weight: 3 },
  { type: 'free-spins', label: '+10 FS', amount: 10, weight: 1 },
...
const translations = {
  en: {
    ...
    minor: 'Minor',
    mini: 'Mini',
    logoTop: 'Deposit to play',
    logoBottom: 'the new Coin Strike 2.0',
    bonusPot: 'Bonus pot',
    minWin: 'Min win',
    timer: 'Timer',
    balance: 'Balance',
  ...
  fr: {
    ...
    minor: 'Mineur',
    mini: 'Mini',
    logoTop: 'Déposez pour jouer',
    logoBottom: 'au nouveau Coin Strike 2.0',
  ...
  de: {
    ...
    minor: 'Minor',
    mini: 'Mini',
    logoTop: 'Zahle ein, um',
    logoBottom: 'den neuen Coin Strike 2.0 zu spielen',
    bonusPot: 'Bonus-Topf',
  ...
  da: {
    ...
    minor: 'Minor',
    mini: 'Mini',
    logoTop: 'Indbetal for at spille',
    logoBottom: 'den nye Coin Strike 2.0',
    bonusPot: 'Bonuspulje',
    minWin: 'Min. gevinst',
    timer: 'Timer',
    balance: 'Saldo',
  ...
  it: {
    ...
    minor: 'Minor',
    mini: 'Mini',
    logoTop: 'Deposita per giocare',
    logoBottom: 'al nuovo Coin Strike 2.0',
  ...
  ru: {
    ...
    minor: 'Минор',
    mini: 'Мини',
    logoTop: 'Внеси депозит чтобы сыграть',
    logoBottom: 'в новый Coin Strike 2.0',
    bonusPot: 'Бонус банк',
    minWin: 'Мин. выигрыш',
    timer: 'Таймер',
    balance: 'Баланс',
...
function buildGrid() {
  ...
  for (let i = 0; i < 9; i += 1) {
    ...
    ui.gridCells.push(cell);
  }
}

function seedGrid() {
  const preset = [
    { type: 'multiplier', label: 'x200', multiplier: 200, tier: 'high' },
    { type: 'multiplier', label: 'x20', multiplier: 20, tier: 'medium' },
    { type: 'multiplier', label: 'x5', multiplier: 5, tier: 'low' },
    { type: 'bonus', label: 'BONUS\n+€100', amount: 100 },
    { type: 'win', label: 'WIN\n€50', amount: 50 },
    { type: 'multiplier', label: 'x3', multiplier: 3, tier: 'low' },
    { type: 'respin', label: 'RE-SPIN' },
    { type: 'free-spins', label: '+5 FS', amount: 5 },
    { type: 'multiplier', label: 'x10', multiplier: 10, tier: 'medium' },
  ];
  updateGridDisplay(preset);
}
...
function updateAdminForm() {
  if (!ui.adminForm) return;
  ui.adminForm.querySelectorAll('[data-admin-field]').forEach((input) => {
...
function pickSymbol() {
  const totalWeight = symbolPool.reduce((sum, symbol) => sum + symbol.weight, 0);
  let ticket = Math.random() * totalWeight;
  for (let i = 0; i < symbolPool.length; i += 1) {
    const symbol = symbolPool[i];
    if (ticket < symbol.weight) {
      return { ...symbol };
    }
...
async function startRecording() {
  ...
  recorder.onstop = () => {
    ...
    if (recordingState.previousUrl) {
      URL.revokeObjectURL(recordingState.previousUrl);
    }
...
async function drawRecordingFrame(target) {
  if (!recordingState.isRecording) return;
  try {
    const snapshot = await window.html2canvas(target, {
  ...
  recordingState.frameRequest = requestAnimationFrame(() => drawRecordingFrame(target));
}
...
function stopRecording(cancelled = false) {
  ...
  if (recordingState.frameRequest) {
    ...
    recordingState.frameRequest = null;
  }
